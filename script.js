...
function clearAll(){ if(confirm('לאפס את היומן?')){ localStorage.removeItem(KEY); render(); } }
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  function paint(){
    ...
    filtered.forEach(r=>{
      ...
      tr.innerHTML = `<td>${r.date}</td><td>${r.start}</td><td>${r.end}</td><td>${formatN(r.hours)}</td><td>${formatN(r.overtime)}</td><td>${formatN(r.pay)}</td><td>${r.note||''}</td><td class="row-actions"><button class="btn btn-ghost" data-id="${r.id}">🗑️</button></td>`;
      tblBody.appendChild(tr);
    });
    document.querySelectorAll('button[data-id]').forEach(b=> b.addEventListener('click', ()=>{ if(confirm('למחוק רשומה?')){ removeEntry(b.dataset.id); paint(); } }));
